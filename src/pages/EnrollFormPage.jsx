...
//         setFormValues(init);
//       } catch (err) {
//         if (err?.response?.status === 404) setError("The requested course could not be found.");
//         else setError("Unable to load enrollment form. Please try again later.");
//       } finally {
//         setLoading(false);
//       }
//     };
...
//     if (field?.type === "file") setFileValue(e.target.files[0] || null);
//     else setFormValues((prev) => ({ ...prev, [field ? field.name : e.target.name]: e.target.value }));
//   };
...
//           <button onClick={() => window.location.reload()} className="px-5 py-2 bg-purple-700 text-white rounded-lg hover:bg-purple-800 transition">Retry</button>
//           <button onClick={() => navigate("/courses")} className="px-5 py-2 border border-purple-700 text-purple-700 rounded-lg hover:bg-purple-50 transition">Back to Courses</button>
//         </div>
//       </div>
//     </div>
//   );
...
//         <h1 className="text-2xl font-bold text-center text-purple-700 mb-6">Enroll — {course.title}</h1>
//         <form onSubmit={handleSubmit} className="space-y-4">
//           {/* Name */}
//           <div>
//             <label className="block text-sm font-medium text-gray-700 mb-1">Name *</label>
//             <input type="text" name="name" value={formValues.name || ""} onChange={handleChange} required className="w-full border rounded px-3 py-2 text-black bg-white" />
//           </div>

//           {/* Email */}
//           <div>
//             <label className="block text-sm font-medium text-gray-700 mb-1">Email *</label>
//             <input type="email" name="email" value={formValues.email || ""} onChange={handleChange} required className="w-full border rounded px-3 py-2 text-black bg-white" />
//           </div>
...
//               {(f.type === "text" || f.type === "email" || f.type === "number" || f.type === "tel") && (
//                 <input type={f.type} value={formValues[f.name] || ""} onChange={(e) => handleChange(e, f)} required={f.required} className="w-full border rounded px-3 py-2 text-black bg-white" />
//               )}
//               {f.type === "textarea" && (
//                 <textarea rows={4} value={formValues[f.name] || ""} onChange={(e) => handleChange(e, f)} required={f.required} className="w-full border rounded px-3 py-2 text-black bg-white" />
//               )}
//               {f.type === "radio" && (
//                 <div className="flex gap-4">
//                   {(f.options || []).map((opt) => (
//                     <label key={opt} className="flex items-center gap-2 text-gray-700">
//                       <input type="radio" name={f.name} value={opt} checked={formValues[f.name] === opt} onChange={(e) => handleChange(e, f)} required={f.required} />
//                       {opt}
//                     </label>
//                   ))}
//                 </div>
//               )}
//               {f.type === "file" && (
//                 <input type="file" accept={f.accept || "*/*"} onChange={(e) => handleChange(e, f)} className="w-full text-black" />
//               )}
//             </div>
//           ))}

//           {statusMsg && <p className="text-center text-sm font-medium text-purple-700">{statusMsg}</p>}

//           <button type="submit" disabled={submitting} className="w-full bg-purple-700 text-white py-2 rounded-lg font-semibold hover:bg-purple-800 transition">{submitting ? "Submitting…" : "Submit Enrollment"}</button>
//         </form>
//       </div>
//     </div>
//   );
