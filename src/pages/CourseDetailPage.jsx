...
const CourseDetailPage = () => {
  ...
  return (
    ...
    >
      ...
      <div className="max-w-4xl mx-auto grid sm:grid-cols-2 gap-6 mb-14">
        {course.duration && <InfoCard icon={<Clock />} label="Duration" value={course.duration} />}
        {course.days && <InfoCard icon={<Calendar />} label="Days" value={course.days} />}
        {course.time && <InfoCard icon={<Clock />} label="Time" value={course.time} />}
        {course.startingDate && <InfoCard icon={<Star />} label="Starting Date" value={course.startingDate} />}
      </div>
      ...
      <div className="max-w-4xl mx-auto space-y-12 text-gray-800">
        {course.contents?.length > 0 && (
          <Section title="Course Contents">
            <ul className="list-disc pl-6 space-y-2">
              {course.contents.map((item, i) => <li key={i}>{item}</li>)}
            </ul>
          </Section>
        )}
        ...
        {course.objectives?.length > 0 && (
          <Section title="Objectives">
            <ul className="list-disc pl-6 space-y-2">
              {course.objectives.map((item, i) => <li key={i}>{item}</li>)}
            </ul>
          </Section>
        )}

        {course.whoCanJoin && <Section title="Who Can Join"><p>{course.whoCanJoin}</p></Section>}
        {course.feeStructure?.length > 0 && <Section title="Fee Structure"><ul className="space-y-1">{course.feeStructure.map((fee,i)=><li key={i}>• {fee}</li>)}</ul></Section>}
        {course.medium && <Section title="Medium"><p>{course.medium}</p></Section>}
        {course.note && <Section title="Note"><p>{course.note}</p></Section>}
        {course.contact && <Section title="Contact"><p>{course.contact}</p></Section>}

        <div className="text-center pt-8">
          <Link
            to={`/enroll/${course.slug}`}
            className="inline-flex items-center gap-3 px-12 py-4 bg-purple-700 text-white text-lg font-bold rounded-xl shadow-lg hover:bg-purple-800 hover:scale-105 transition"
          >
            Enroll Now <ArrowRight />
          </Link>
        </div>
...
const Section = ({ title, children }) => (
  <section>
    <h2 className="text-3xl font-extrabold text-purple-900 mb-3">{title}</h2>
    <div className="text-lg leading-relaxed">{children}</div>
  </section>
);